...
namespace LiberatingMarsCLI
{
    class Converter
    {
        public byte[] ctbContent = null;
        public byte[] outContent = null;

        //long oldHeaderFinalPos = 0;
        long disclaimerOffPos = 0;
        long disclaimerPos = 0;

        OldHeader oldH = new OldHeader();
        NewHeader newH = new NewHeader();

        OldLayers oldLay = new OldLayers();
        NewLayers newLay = new NewLayers();

        public byte[] AESKey = new byte[32];
        public byte[] AESIV = new byte[16];

        public Converter(byte[] content, byte[] aes_key, byte[] aes_iv)
        {
            ctbContent = content;
            AESKey = aes_key;
            AESIV = aes_iv;
        ...
        public void translateHeader()
        {
            ...
            outContent = outStream.ToArray();

            Console.WriteLine("translateHeader -- outContent.length: " + outContent.Length);
        ...
        public void translateLayers()
        {
            MemoryStream Stream = new MemoryStream(ctbContent);
            BinaryReader binRead = new BinaryReader(Stream);

            MemoryStream outStream = new MemoryStream();
            outStream.Write(outContent, 0, outContent.Length);
            BinaryWriter binWrite = new BinaryWriter(outStream);
            binWrite.Seek(0, SeekOrigin.End);

            Console.WriteLine("translateLayers -- binWrite.BaseStream.Position: " + binWrite.BaseStream.Position);
            ...
            long length = binWrite.BaseStream.Length;
            binWrite.BaseStream.Position = 0x18;
            binWrite.Write(length);
        ...
        public void saveNewCtb(bool skip)
        {
            sign(skip);
            encrypt(skip);
        }

        public void sign(bool skip)
        {
            ...
            if (skip)
                inStream.Write(ctbContent, 0, ctbContent.Length);
            else
                inStream.Write(outContent, 0, outContent.Length);

            BinaryReader read = new BinaryReader(inStream);
            ...
            var EncHash = AES.AesCryptBytes(hashed8Bytes, true, AESKey, AESIV);
            encryptedHashed = EncHash;

            MemoryStream outStream = new MemoryStream();

            if (skip)
                outStream.Write(ctbContent, 0, ctbContent.Length);
            else
                outStream.Write(outContent, 0, outContent.Length);

            BinaryWriter write = new BinaryWriter(outStream);
        ...
        public void encrypt(bool skip)
        {
            ...
            encHeader = AES.AesCryptBytes(header, true, AESKey, AESIV);
            read.Close();
